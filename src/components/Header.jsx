          ...
          {navigation.map((item) => (
            <Link
              ...
              className={`text-2xl font-medium ${
                location.pathname === item.href ? 'text-orange-light' : 'text-white hover:text-orange-light'
              }`}
            >
              {item.name}
            </Link>
        ...
        <DialogPanel className="fixed inset-y-0 right-0 z-10 w-full overflow-y-auto bg-black px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-white/10">
          ...
          <div className="mt-6 flow-root">
            <div className="-my-6 divide-y divide-gray-500/25">
              <div className="space-y-2 py-6 ml-10">
                {navigation.map((item) => (
                  <Link
                    key={item.name}
                    to={item.href}
                    onClick={() => setMobileMenuOpen(false)}
                    className={`-mx-3 block rounded-lg px-3 py-2 text-2xl font-medium ${
                      location.pathname === item.href ? 'text-orange-light' : 'text-white hover:text-orange-light'
                    }`}
                  >
                    {item.name}
